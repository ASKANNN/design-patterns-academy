...
- **Skip link.** `index.html` provides a "Skip to content" link targeting
  `#main-content`.
- **User-facing accessibility controls.** `AccessibilityWidget`
  (`src/components/ui/AccessibilityWidget.js`, state in
  `src/config/accessibility.js`) lets visitors adjust font size, contrast mode
  (normal/high/invert), monochrome, a readable font, and underlined links,
  with a reset — independent of the theme toggle.
