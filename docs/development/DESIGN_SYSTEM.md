...
Each `--shadow-*` token is redefined per theme in `tokens.css` (light values
below; dark theme uses higher-opacity black shadows for the same tokens).

| Token | Value | Usage |
|-------|-------|-------|
| `--shadow-none` | `none` | Flat elements |
| `--shadow-xs` | `0 1px 2px rgba(0,0,0,0.05)` | Subtle lift |
| `--shadow-sm` | `0 1px 2px rgba(0,0,0,0.04), 0 4px 8px rgba(0,0,0,0.04)` | Cards (default) |
| `--shadow-md` | `0 2px 4px rgba(0,0,0,0.04), 0 10px 20px rgba(0,0,0,0.06)` | Hover cards |
| `--shadow-lg` | `0 4px 8px rgba(0,0,0,0.04), 0 20px 40px rgba(0,0,0,0.08)` | Dropdowns |
| `--shadow-xl` | `0 8px 16px rgba(0,0,0,0.04), 0 32px 64px rgba(0,0,0,0.10)` | Modals |
...
### Library
- No external icon library or dependency — the project has zero runtime deps.
- Format: hand-authored inline SVG strings, `viewBox="0 0 24 24"`
  (`src/components/visual/PatternIcon.js`'s glyph set, `IconButton.js`'s raw
  SVG prop)
...
### Rules
- `stroke-width` consistent across an icon's own strokes (per glyph, not a global library rule)
